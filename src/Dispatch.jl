...
function distribute{T<:Integer}(fbndr::Function, fbndc::Function, frhs::Function, Nx::T, Ny::T, M::T)::Dict{Array{Int,1}, Patch}
    ...
    for i in 2:2M, k in i-min(i-1,M):min(i-1,M) 
        ...
        bndx = (loc[2]==1) ? getPatchICwoProjection(fbndr, 0, Nx, M, loc[1]) : getPatchBnd(dbase[loc-[0,1]], 0)
        bndy = (loc[1]==1) ? getPatchICwoProjection(fbndc, 1, Ny, M, loc[2]) : getPatchBnd(dbase[loc-[1,0]], 1)
